<?php 
    ...
    // $_POST itu method di formnya    
     if(isset($_POST['loginPemilikMobil'])){  
    ...
    "SELECT * FROM pemilikmobil WHERE namapemilikmobil = '$namapemilikmobil'") or die(mysqli_error($con));         
// ini buat ngecek kalo misalnya hasil dari querynya == 0 ato ga ketemu -> usernamenya tdk ditemukan         
    if(mysqli_num_rows($query) == 0)
    {              
        echo 
            '<script>                 
                alert("Pemilik Mobil not found!"); 
                window.location = "../page/loginPemilikMobil.php"                 
            </script>'; 
    }
    else
    { 
        ...
        if(password_verify($password, $user['password'])){ 
            ...
            echo 
                '<script>                     
                    alert("Login Success"); 
                    window.location = "../page/dashboardPemilikMobilPage.php"                     
                </script>'; 
            }
            else 
            {                 
