<?php 
    ...
    "SELECT * FROM jabatan WHERE namajabatan = '$namajabatan'") or die(mysqli_error($con));         
// ini buat ngecek kalo misalnya hasil dari querynya == 0 ato ga ketemu -> usernamenya tdk ditemukan         
    if(mysqli_num_rows($query) == 0)
    {              
        echo 
            '<script>                 
                alert("Jabatan not found!"); 
                window.location = "../page/loginJabatan.php"                 
            </script>'; 
    }
    else
    { 
        $user = mysqli_fetch_assoc($query);             
            ...
            echo 
                '<script>                     
                    alert("Login Success"); 
                    window.location = "../page/registerPegawai.php"                     
                </script>'; 
